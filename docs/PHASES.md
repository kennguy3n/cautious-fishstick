...
- [x] Connector health dashboard in Admin UI: per-connector last-sync time, error count, credential expiry. *(Last open exit criterion in this phase — retained as `[ ]` until the Admin UI surface ships in `ztna-frontend`.)*

---

## Phase 8 — Workflow orchestration  🟡 partial
...
- [x] Multi-step approval workflows with conditional routing (e.g. `manager → resource-owner → security`). *(`WorkflowExecutor.Execute` walks `access_workflows.steps` linearly; each step returns `approve` / `deny` / `escalate` / `pending`. Step types: `auto_approve`, `manager_approval`, `security_review`, `multi_level`. PR #19. The full LangGraph DAG runtime with branch fan-out is still ⏳ — the Phase 8 executor is sufficient for linear pipelines.)*
...
## Phase 10 — Connector scale-out (200 connectors)  🟡 partial
