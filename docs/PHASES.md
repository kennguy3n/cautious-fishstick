...
- [x] **Finance** (4): QuickBooks, Xero, Stripe, FreshBooks. *(All 4 land in PR #11 — QuickBooks `/v3/company/{realm}/query` with `SELECT * FROM Employee STARTPOSITION/MAXRESULTS` and OAuth2 bearer; Xero `/api.xro/2.0/Users` with `Xero-Tenant-Id` header + offset pagination; Stripe `/v1/team_members` with `starting_after` cursor + `has_more`; FreshBooks `/accounting/account/{account_id}/users/staffs` with page/per_page.)*
