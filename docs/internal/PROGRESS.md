...
| Outbound SCIM | ✅ | Phase 6. Generic SCIM v2.0 client with per-connector `SCIMProvisioner` composition across 8 of the 10 Tier-1 connectors (Microsoft Entra ID, Google Workspace, Okta, Auth0, Duo Security, 1Password, LastPass, Ping Identity); the two generic protocol stubs (`generic_saml`, `generic_oidc`) have no upstream API to push SCIM to and are intentionally n/a. Count locked in by `TestRegistry_SCIMProvisionerCount`. |
...
| API hardening | ✅ | Per-workspace token-bucket rate limiting (`ZTNA_API_RATE_LIMIT_RPS`), JSON request validation middleware, request-ID middleware (UUIDv4 minted or inbound `X-Request-ID` echoed, threaded into logs + `request_id` field of every error envelope), credential-rotation alerts, batch connector status endpoint, and an audited OpenAPI 3.0 spec. |
...
| SSO-only enforcement verification (Phase 11) | ✅ | `SSOEnforcementChecker` capability + 14 implementations (Salesforce, Google Workspace, Okta, Slack, GitHub, Microsoft, Auth0, Ping Identity, Zendesk, BambooHR, Workday, HubSpot, Dropbox, Zoom). Surfaced through the connector health endpoint and locked in by `TestRegistry_SSOEnforcementCheckerCount`. |
| Session revocation (Phase 11) | ✅ | `SessionRevoker` capability + 14 implementations (Okta, Google Workspace, Microsoft, Salesforce, Slack, Auth0, GitHub, Zoom, Zendesk, HubSpot, Dropbox, Jira/Atlassian, Notion, BambooHR). Invoked from the enhanced leaver flow and locked in by `TestRegistry_SessionRevokerCount`. 14 is treated as the realistic ceiling for now (every provider with a documented session-revocation API has a connector); growth requires a paired API + connector change rather than further docs work. |
| Six-layer leaver kill switch (Phase 11) | ✅ | `JMLService.HandleLeaver` now revokes grants → removes memberships → disables Keycloak user → revokes upstream sessions → SCIM-deprovisions → disables OpenZiti identity. All steps best-effort and idempotent. |
...
| Date | What | PR |
|------|------|----|
| 2026-05-15 | 30-task hardening batch — Groups A + D (docs, infra) shipped doc cross-ref sweep (117 files), `make stale-ref-check` CI guard, `make test-integration` target, `docker/README.md`, `cmd/README.md`, and the `.github/workflows/integration-test.yml` Postgres workflow. Group C (HTTP hardening) added `internal/handlers/request_id_middleware.go` (UUID-per-request + `X-Request-ID` header + `request_id` in error envelopes), per-binary healthcheck tests for `cmd/ztna-api` + `cmd/access-workflow-engine`, and the `internal/migrations/migrations_integration_test.go` 15-migration round-trip test. Group B (test coverage) added `internal/services/access/connector_management_service_test.go`, `internal/services/access/sso_federation_service_integration_test.go`, `internal/cron/anomaly_scanner_integration_test.go`, `internal/services/access/impact_resolver_edge_test.go`, `internal/config/access_validation_test.go`, and `internal/handlers/router_integration_test.go` (route status / middleware chain). Swagger docs regen for the new `request_id` field + `X-Request-ID` response header. | #88 + #89 + (this PR) |
| 2026-05-15 | Group C — Phase 11 promoted to ✅ across `PHASES.md` / `PROGRESS.md`; cross-doc terminology fix five-layer → six-layer leaver kill switch; SCIM 10 → 8 audit note (`generic_saml` + `generic_oidc` n/a — no upstream API). Zoom `AccountID` URL-escape consistency follow-up landed separately in #84. | #87 |
| 2026-05-15 | Group B — `SSOEnforcementChecker` for Dropbox + Zoom (12 → 14, finally matches README), `Makefile` + `CONTRIBUTING.md` seven-test recipe, registry count + orphan-directory guards (`internal/services/access/registry_count_test.go`), optional-interface count guards (`SCIMProvisioner=8`, `GroupSyncer=5`, `IdentityDeltaSyncer=3`, `AccessAuditor=198`), logging-middleware hardening tests (no request body / sensitive headers / duration / unmatched-route bucketing), `docker-compose.yml` healthcheck guard (`internal/docker/healthcheck_test.go`). | #81 |
| 2026-05-15 | Group A — dedicated test files for `access_provision_test.go`, `access_revoke_test.go`, `sso_metadata_helpers_test.go`, `notification_adapter_test.go`, `connector_credentials_loader_test.go`, `cmd/ztna-api/main_test.go`, `cmd/access-workflow-engine/main_test.go`, `review_automator_test.go`, `registry_count_test.go`, `errors_test.go`, `orphan_reconciler_http_dryrun_test.go`, plus grant-expiry warn→revoke interaction test. | #80 |
...
| 2026-05-14 | Phase 11 — Hybrid Access Model: per-connector access_mode classification, unused-app-account reconciler, SSO-only enforcement verification, session revocation across top-7 connectors, six-layer leaver kill switch, automatic grant-expiry enforcement, plus full docs sync. | #77 |
