...
- SSO federation service: `internal/services/access/sso_federation_service.go` (`KeycloakClient` interface + `HTTPKeycloakClient` + `SSOFederationService.Configure` / `ConfigureBroker` mapping `SSOMetadata` to Keycloak SAML / OIDC IdP config; wired into `ConnectAccessConnector` for **185 connectors** with explicit broker tests in `sso_federation_service_test.go`: Microsoft Entra ID, Google Workspace, Generic SAML, Generic OIDC, Okta, Ping Identity (PR #25 originals), BambooHR, Workday, Zendesk, Dropbox Business, Salesforce, GitHub, Auth0, GitLab, Jira, Slack Enterprise, MS Teams (PR #25 batch 2), Cloudflare Access, Rippling, ForgeRock, Keeper, OpenAI Enterprise (PR #29 batch 3), AWS IAM Identity Center, Azure Entra ID, GCP Workforce Identity Federation (PR #31 batch 4), **SAP Concur, Coupa, LinkedIn Learning, Udemy Business (all SAML), RingCentral (OIDC) — batch 5 via the new shared helper `internal/services/access/sso_metadata_helpers.go::SSOMetadataFromConfig(configRaw, defaultProtocol)` that reads `sso_metadata_url` / `sso_entity_id` / `sso_login_url` / `sso_logout_url` from the connector's `configRaw` and returns `nil` when the metadata URL is blank, so callers gracefully downgrade to `ErrSSOFederationUnsupported`; HubSpot, Notion, Box, PagerDuty, Sentry (all SAML) — batch 6 via the same `SSOMetadataFromConfig` helper; JFrog, LaunchDarkly, New Relic, Splunk Cloud, Sumo Logic (all SAML) — batch 8 (PR #34); Datadog, Freshdesk, Front, Asana, Monday.com (all SAML) — batch 9 (PR #35); Figma, Miro, Airtable, Smartsheet, ClickUp (all SAML) — batch 10 (PR #36); Zoho CRM, Egnyte, KnowBe4, Docker Hub, Terraform Cloud (all SAML) — batch 11 (PR #37); Crisp, Shopify, NetSuite, Coursera (all SAML) — batch 12 (PR #38) — 4 net new wires (Slack Enterprise was already SAML-wired in batch 9); DocuSign, DocuSign CLM, Google Gemini (OIDC), Gusto, Hibob (all SAML) — batch 13 (PR #39); Hootsuite, Sprout Social, Buffer, Magento, Square (all SAML) — batch 14 (PR #40); Twilio, Sendgrid, Vonage, Shopify (re-verified), WordPress (all SAML) — batch 16 (PR #41); **Tailscale (OIDC), Heroku, DigitalOcean, Vercel, Netlify (all SAML) — batch 17 (PR #43) via the same `SSOMetadataFromConfig` helper**. Zoom returns `ErrSSOFederationUnsupported` (no native SSO metadata API).
...
The contracts live in-tree under [`sdk/`](../sdk):

- [`sdk/ios/`](../sdk/ios) — Swift Package. `AccessSDKClient` protocol in [`Sources/ShieldNetAccess/AccessSDKProtocol.swift`](../sdk/ios/Sources/ShieldNetAccess/AccessSDKProtocol.swift), data models in [`Sources/ShieldNetAccess/Models.swift`](../sdk/ios/Sources/ShieldNetAccess/Models.swift), contract-conformance test in [`Tests/ShieldNetAccessTests/ContractTests.swift`](../sdk/ios/Tests/ShieldNetAccessTests/ContractTests.swift).
- [`sdk/android/`](../sdk/android) — Kotlin library. `AccessSDKClient` interface + data classes in [`src/main/kotlin/com/shieldnet360/access/AccessSDKClient.kt`](../sdk/android/src/main/kotlin/com/shieldnet360/access/AccessSDKClient.kt), contract-conformance test in [`src/test/kotlin/com/shieldnet360/access/ContractTest.kt`](../sdk/android/src/test/kotlin/com/shieldnet360/access/ContractTest.kt).
- [`sdk/desktop/`](../sdk/desktop) — Electron extension. `AccessIPC` interface + `AccessIPCChannel` constants in [`src/access-ipc.ts`](../sdk/desktop/src/access-ipc.ts), contract-conformance test in [`src/__tests__/contract.test.ts`](../sdk/desktop/src/__tests__/contract.test.ts).

The cross-cutting "no on-device inference" rule is enforced in CI by [`scripts/check_no_model_files.sh`](../scripts/check_no_model_files.sh), which fails the build if any `.mlmodel`, `.tflite`, `.onnx`, or `.gguf` file lands under `sdk/`. See [`docs/SDK_CONTRACTS.md`](SDK_CONTRACTS.md) for the canonical reference of all three contracts.
...
**Local dev option (PR #67):** the same six processes (Postgres, Redis, `ztna-api`, `access-connector-worker`, `access-workflow-engine`, `access-ai-agent`) can be brought up locally via `docker compose up --build --wait` against the repo-root `docker-compose.yml`. Each Go service builds from `docker/Dockerfile.*` (multi-stage `golang:1.25-alpine` → `gcr.io/distroless/static-debian12:nonroot`); the Python `access-ai-agent` builds from `cmd/access-ai-agent/Dockerfile` (`python:3.12-slim`). Healthchecks wire `pg_isready` / `redis-cli ping` / the in-binary `/health` (Go services) / `/healthz` (Python agent) so `--wait` blocks until the stack is actually responsive. The dev stack is not a substitute for the per-runtime resource profile above — it exists only so contributors can exercise the full request flow end-to-end without a Kubernetes cluster.

**Kubernetes deployment:** the four runtime services are also packaged for cluster deployment under `deploy/k8s/` (raw manifests + Kustomize entry-point) and `deploy/helm/shieldnet-access/` (Helm chart). Both paths render the same 21-resource bundle: a `shieldnet-access` namespace, four Deployments, four ConfigMaps + Secrets, four ServiceAccounts, three Services (`ztna-api`, `access-workflow`, `access-ai-agent` — the worker is a headless queue consumer), and an HPA for `ztna-api` (2–10 replicas, 70% CPU target). Resource limits encode the per-runtime profile above; readiness/liveness probes hit `/health` on the Go services and `/healthz` on the Python agent. Per `docs/PROPOSAL.md` §10.3 only `ztna-api` is exposed publicly via the platform's ingress; the workflow engine and AI agent stay cluster-internal and authenticate via the shared `X-API-Key` header pinned by `ACCESS_AI_AGENT_API_KEY`. Chart structure mirrors `uneycom/ztna-k8s-assets` and `uneycom/shieldnet-agents-k8s-helm` (reference-only repos).
