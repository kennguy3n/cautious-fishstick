...
- SSO federation service: `internal/services/access/sso_federation_service.go` (`KeycloakClient` interface + `HTTPKeycloakClient` + `SSOFederationService.Configure` / `ConfigureBroker` mapping `SSOMetadata` to Keycloak SAML / OIDC IdP config; wired into `ConnectAccessConnector` for **22 connectors** with explicit broker tests in `sso_federation_service_test.go`: Microsoft Entra ID, Google Workspace, Generic SAML, Generic OIDC, Okta, Ping Identity (PR #25 originals), BambooHR, Workday, Zendesk, Dropbox Business, Salesforce, GitHub, Auth0, GitLab, Jira, Slack Enterprise, MS Teams (PR #25 batch 2), Cloudflare Access, Rippling, ForgeRock, Keeper, OpenAI Enterprise (PR #29 batch 3). Zoom returns `ErrSSOFederationUnsupported` (no native SSO metadata API).
