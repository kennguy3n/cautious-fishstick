...
- SSO federation service: `internal/services/access/sso_federation_service.go` (`KeycloakClient` interface + `HTTPKeycloakClient` + `SSOFederationService.Configure` / `ConfigureBroker` mapping `SSOMetadata` to Keycloak SAML / OIDC IdP config; wired into `ConnectAccessConnector` for Microsoft Entra ID, Google Workspace, Generic SAML, Generic OIDC, Okta, Ping Identity, plus 11 connectors with explicit broker tests in `sso_federation_service_test.go`: BambooHR, Workday, Zendesk, Dropbox Business, Salesforce, GitHub, Auth0, GitLab, Jira, Slack Enterprise, MS Teams. Zoom returns `ErrSSOFederationUnsupported`).
