...
| SSO-only enforcement verification (Phase 11) | 🟡 | `SSOEnforcementChecker` capability + 14 implementations (Salesforce, Google Workspace, Okta, Slack, GitHub, Microsoft, Auth0, Ping Identity, Zendesk, BambooHR, Workday, HubSpot, Dropbox, Zoom). Surfaced through the connector health endpoint. |
...
| Date | What | PR |
|------|------|----|
| 2026-05-15 | Group B — `SSOEnforcementChecker` for Dropbox + Zoom (12 → 14, matches README claim), `Makefile` + `CONTRIBUTING.md`, registry count + orphan-directory guards (`internal/services/access/registry_count_test.go`), optional-interface count guards (`SCIMProvisioner=8`, `GroupSyncer=5`, `IdentityDeltaSyncer=3`, `AccessAuditor=198`), logging-middleware hardening tests (no request body / sensitive headers / duration / unmatched-route bucketing), `docker-compose.yml` healthcheck guard (`internal/docker/healthcheck_test.go`). | this PR |
