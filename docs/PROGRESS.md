...
- **What landed in PR #19.** `cmd/access-workflow-engine/` is now a real HTTP host listening on `:8082` (configurable via `ACCESS_WORKFLOW_ENGINE_LISTEN_ADDR`). The engine exposes `GET /health` (liveness) and `POST /workflows/execute` (executes a workflow against a request). The `internal/services/access/workflow_engine` package implements `WorkflowExecutor` with four step types — `auto_approve`, `manager_approval`, `security_review`, and `multi_level` — each a function returning `approve` / `deny` / `escalate` / `pending`. Risk routing lives in `RiskRouter.Route(risk, tags)` and is integrated with `WorkflowService.ResolveWorkflowWithRisk` (low → self_service / auto_approve, medium → manager_approval, high or `sensitive_resource` risk factor → security_review, unknown → manager_approval fail-safe). Escalation lives in `EscalationChecker.Run`, a cron that scans pending requests and emits `Escalator.Escalate(from, to)` calls when the first step's `timeout_hours` has elapsed since `UpdatedAt`. The `access_workflows.steps` JSON schema gains `timeout_hours`, `escalation_target`, and `levels[]` for `multi_level` step types — all backward-compatible with the Phase 2 short form.
- **Still open.** Real LangGraph DAG runtime (the Phase 8 executor walks steps linearly; multi-branch graphs still ⏳); durable step state (the executor is currently stateless within a single Execute call — pending requests rely on `access_requests.state` for resume); retry / dead-letter queue for transient executor errors; the production Escalator implementation that writes to `access_request_state_history` and dispatches notifications (currently a logging-only stub in the binary).
