...
| AI Review Automation Agent | 🟡 | Phase 5 — Python `access_review_automation` stub under `cmd/access-ai-agent/skills/access_review_automation.py` (PR #7); Go-side wiring from `AccessReviewService` to flip pending → certify still ⏳ |
...
| AI Anomaly Detection Agent | 🟡 | Phase 6 — Go-side `AIClient.DetectAnomalies` + `DetectAnomaliesWithFallback` + `AnomalyDetectionService.ScanWorkspace` AND Python `access_anomaly_detection` stub (PR #7); cross-grant baseline histogram still ⏳ |
...
| Access Review Campaigns | 🟡 | Phase 5 — tables, models, `StartCampaign` / `SubmitDecision` / `CloseCampaign` / `AutoRevoke` (PR #5); HTTP handlers `POST /access/reviews`, `POST /access/reviews/:id/decisions|close|auto-revoke` and `internal/cron.CampaignScheduler` driving `access_campaign_schedules` (PR #6); auto-certification rate metric (`GET /access/reviews/:id/metrics`), admin `PATCH /access/reviews/:id`, and the notification fan-out from `StartCampaign` ship in PR #7; AI auto-certification + email / Slack channels still ⏳ |
| Notification System | 🟡 | Phase 5 — `internal/services/notification.NotificationService`, `Notifier` interface, in-memory channel, `NotifyReviewersPending` / `NotifyRequester` (PR #7); email / Slack / push channels still ⏳ |
| JML Automation | 🟡 | Phase 6 — `internal/services/access.JMLService` with `ClassifyChange` / `HandleJoiner` / `HandleMover` (atomic batch, no partial-access window) / `HandleLeaver` (PR #7); inbound SCIM handler routing into JML ships in the same PR; OpenZiti identity disable on leaver still ⏳ |
| Outbound SCIM | 🟡 | Phase 6 — `internal/services/access.SCIMClient` (generic SCIM v2.0 push, `PushSCIMUser` / `PushSCIMGroup` / `DeleteSCIMResource`) with sentinel errors for 409 / 404 / 401 / 5xx and idempotent delete (PR #7); per-connector composition into the existing connectors still ⏳ |
