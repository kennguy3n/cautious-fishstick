...
### 3.3 Workflow Orchestration (Phase 8 — remaining work)

- **What landed in PR #19.** `cmd/access-workflow-engine/` is a real HTTP host listening on `:8082` (configurable via `ACCESS_WORKFLOW_ENGINE_LISTEN_ADDR`). The engine exposes `GET /health` (liveness) and `POST /workflows/execute` (executes a workflow against a request). The `internal/services/access/workflow_engine` package implements `WorkflowExecutor` with four step types — `auto_approve`, `manager_approval`, `security_review`, and `multi_level` — each a function returning `approve` / `deny` / `escalate` / `pending`. Risk routing lives in `RiskRouter.Route(risk, tags)` and is integrated with `WorkflowService.ResolveWorkflowWithRisk`. Escalation lives in `EscalationChecker.Run`, a cron that scans pending requests and emits `Escalator.Escalate(from, to)` calls when the first step's `timeout_hours` has elapsed since `UpdatedAt`.
- **What landed in this batch.** `RealStepPerformer` (`internal/services/access/workflow_engine/performer.go`) replaces `NoOpPerformer` and drives the four step types against the real `AccessRequestService`. `NotifyingEscalator` (`notification_escalator.go`) replaces `loggingEscalator`: it writes an `AccessRequestStateHistory` row recording the escalation event and fans a best-effort notification to the escalation target. Default workflow templates (`new_hire_onboarding`, `contractor_onboarding`, `role_change`, `project_access`) seed via migration `008`. Durable step state lands in `access_workflow_step_history` (model + migration `009`); `WorkflowExecutor.Execute` writes one row per step with status (`pending` / `completed` / `failed` / `escalated` / `denied`), attempts, and timestamps. Retry / DLQ logic (`retry.go`) wraps each step with exponential backoff (3 attempts, 100ms → 200ms → 400ms cap 5s) and `WorkflowExecutor.ListFailedSteps` exposes the dead-letter view for operator triage.
- **Still open.** Real LangGraph DAG runtime (the executor walks steps linearly; multi-branch graphs still ⏳); the Admin UI's workflow inspector + DLQ surface.
...
- **What landed in this batch.** AI auto-certification: `ReviewAutomator` calls the `access_review_automation` skill via `AIClient` for every enrolled grant when `auto_certify_enabled=true`; certify verdicts flip the decision to `auto_certified=true`, escalate / error verdicts leave the decision pending for human review. Scheduled-campaign skip dates: `AccessCampaignSchedule.SkipDates` (JSON array of `YYYY-MM-DD` strings) — `CampaignScheduler.Run` advances `NextRunAt` by `FrequencyDays` without launching a campaign when today matches. WebPush notification channel: `internal/services/notification.WebPushNotifier` posts the `pushEnvelope` JSON to each `PushSubscription` endpoint (`push_subscriptions` table + model + migration `010`); failures are logged but never roll back the parent transaction.
