...
make stale-ref-check  # fail if a retired doc filename or bare shorthand (PROPOSAL.md / ARCHITECTURE.md / LISTCONNECTORS.md / SDK_CONTRACTS.md, OR `PROPOSAL §X` / `ARCHITECTURE §X` / `LISTCONNECTORS §X` / `SDK_CONTRACTS §X` without `.md`) is referenced anywhere in the tree
```
