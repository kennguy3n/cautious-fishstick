...
// lifecycle per docs/architecture.md §8 and docs/architecture.md §9.
//
...
//     provisions (for gained Teams). docs/architecture.md calls this out as
...
// lane (or vice-versa).
//
// The rules (per docs/architecture.md):
//
...
// access_requests, fan-out provisioning. Per docs/architecture.md the
...
// lost Teams) + provisions (for gained Teams). Per docs/architecture.md
...
func (s *JMLService) HandleMover(ctx context.Context, in MoverInput) (*JMLResult, error) {
	...
	// lost-team grants. docs/architecture.md requires "no partial-access
...
// OpenZiti identity. Per docs/architecture.md leaver is synchronous — the
...
// team membership is updated atomically (per docs/architecture.md "no
