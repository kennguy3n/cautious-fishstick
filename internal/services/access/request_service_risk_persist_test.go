...
// the contract from docs/architecture.md §9: a failure to persist the AI risk
...
func TestCreateRequest_RiskPersistFailureLogsAndReturnsNilError(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("CreateRequest returned err = %v; want nil (per docs/architecture.md §9 AI is decision-support)", err)
	}
	if got == nil {
