...
// trigger a fresh AccessReview. The production implementation is
// *access.AccessReviewService; tests substitute a stub.
type CampaignStarter interface {
	StartCampaign(ctx context.Context, in access.StartCampaignInput) (*models.AccessReview, []models.AccessReviewDecision, error)
}
...
// per-row work so a transient panic mid-loop does not double-fire
// the same schedule.
//
...
// bumps NextRunAt. Errors from the AccessReviewService are returned
// without bumping NextRunAt so the next Run will retry. The DueAt
// stamped onto the new review is now + defaultDue.
func (s *CampaignScheduler) runOne(ctx context.Context, sched *models.AccessCampaignSchedule, now time.Time) error {
	...
	if _, _, err := s.starter.StartCampaign(ctx, in); err != nil {
