...
- **`pam-gateway` Kubernetes manifests** (`deploy/k8s/pam-gateway/`) — Deployment, ClusterIP Service, ConfigMap, and Kustomize overlay; pinned at `cpu=500m / mem=512Mi`, replay buffer on an `emptyDir` at `/var/lib/shieldnet/replay`, protocol-listener ports and S3 replay-store config sourced from the ConfigMap.
