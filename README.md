...
│   └── check_no_model_files.sh    # Cross-cutting CI guard — fails if any .mlmodel / .tflite / .onnx / .gguf file lands under sdk/. Enforces PROPOSAL.md §11 "no on-device inference" rule. Driven from go test via scripts/check_no_model_files_test.go.
├── blog/                          # Public-facing release notes (12 posts: 00-introducing-shieldnet-access through 11-…)
├── .github/workflows/             # CI: ci.yml (Go build/vet/test -race/swagger --check/sn360-language/sdk-no-model-files) and python-ci.yml (cmd/access-ai-agent pytest)
...
| [`docs/SDK_CONTRACTS.md`](docs/SDK_CONTRACTS.md) | Mobile SDK (iOS Swift + Android Kotlin) and Desktop Extension (Electron TypeScript) API contracts. Tabular `method ↔ REST endpoint` mapping, per-platform contract file pointers, and how the "no on-device inference" rule is verified by `scripts/check_no_model_files.sh`. |
