...
- **SSO-only enforcement verification** — connectors that federate through Keycloak optionally implement `SSOEnforcementChecker`, so the health endpoint can surface "SSO-only mode is OFF" warnings the operator can fix without leaving the dashboard. **14 connectors** ship today: Salesforce, Google Workspace, Okta, Slack, GitHub, Microsoft, Auth0, Ping Identity, Zendesk, BambooHR, Workday, and HubSpot.
