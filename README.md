...
│   └── access_audit.go            # Phase 10 AccessAudit worker handler — loads cursor from access_sync_state (kind=audit), invokes FetchAccessAuditLogs, publishes batches via AuditProducer, advances cursor monotonically, soft-skips ErrAuditNotAvailable
├── internal/handlers/
