...
docs/architecture.md tracks the future "on-device fallback" optionality as a deferred decision. The SDK surface is designed so that adding a local fallback later is a non-breaking change — but adding it is not on any current roadmap.
